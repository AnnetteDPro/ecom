...
La date du dernier achat. Notez bien que l’on part du principe qu’une personne qui a acheté récemment chez vous a plus de chances de revenir commander chez nous.: 
    ...
    st.markdown("""
Les champions reviennent aux moyennes tous les 1-2 mois, client fidèle tous les 3 mois les clients prometteurs 1 fois par ans etc.
            """
    )    
...
La somme des achats cumulés sur une période donnée. Les gros acheteurs sont le meilleur générateur de revenus pour la plate-forme de commerce électronique. On mesure ici la valeur client. Aux moyennes, les gens dépensent sur la plateforme 151 dollars, mais nous avons des gros (plus de 10 000 dollars) et des petits acheteurs (moins de 100 dollars):
    ...
    st.markdown("""
La distribution des segments RFM est suivante:
            """
    )
...
- les offres ciblées tous les mois pour les clients à risque et clients prometteurs, les bons d’achat pour les clients perdus. 
    ...
    st.markdown("""
- Le cross-selling et le up-selling sont en effet la meilleure façon d’augmenter le montant du panier moyen de votre e-commerce, campagne adwords, emailing,  la publicité display en utilisant une DMP et des procédures de CRM onboarding) , retargeting  avec des suggestions de produits complémentaires afin de réaliser des ventes additionnelles de manière fluide et naturelle.(exemple : machine café + dosettes, l’ordinateur / souris, proposition d’un look complet). 
    ...
    st.markdown("""
Plus d’information et plus d’idées, on va avoir en croisant nos segment avec des catégories :
